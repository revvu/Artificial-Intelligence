...
def incremental(total_node_count, avg_degree, node_lst):
    ...
    for node_index in range(starter_edges+1):
        node_lst[node_index]+=starter_edges+1

    node_count=starter_edges+1
    current_edge_count = starter_edges*node_count//2


    for node_index in range(starter_edges+1, len(node_lst)):

        edge_count = int((node_count+1)*avg_degree-2*current_edge_count)//2
        edge_count=min(edge_count,node_count)
        current_edge_count+=edge_count

        randlst = random.choices(range(len(node_lst)), weights=node_lst, k=edge_count)

        for num in randlst:
            node_lst[num]+=1
        node_lst[node_index]=edge_count+1
...
def main():
    ...
    else: incremental(node_count, avg_degree, node_lst)
