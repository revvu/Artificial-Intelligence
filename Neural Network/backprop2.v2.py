...
def train_backprop_batch(input_lst, target_lst, weights):

    adjust_lst = [[0]*len(weights[i]) for i in range(len(weights))]
    for index in range(len(input_lst)):
        inputs = input_lst[index]
        layered_x = [inputs[::]]
        # no activation on the last layer
        for layer in weights[:-1]:
            n = len(layered_x[-1])
        ...
        #error
        error = target_lst[index]-final_output
        #backprop final layer
        adjust_lst[2][0] += alpha*error*layered_x[-1][0]
        #backprop penultimate layer
        for i in range(len(weights[1])):
            adjust_lst[1][i] += alpha*error*weights[2][0]*layered_x[-1][0]*(1-layered_x[-1][0])*layered_x[-2][i]
        #backprop deepest layer
        for j in range(first_hidden_layer):
            for i in range(3):
                adjust_lst[0][i+j*3]+=alpha*error*weights[2][0]*layered_x[-1][0]*(1-layered_x[-1][0])*weights[1][j]*layered_x[-2][j]*(1-layered_x[-2][j])*layered_x[-3][i]

    for layer_index in range(len(weights)):
        for index in range(len(weights[layer_index])):
            weights[layer_index][index]+=adjust_lst[layer_index][index]/len(input_lst)
...
def main():
    ...
    count = 0
    for i in range(input_node_count//10):
        input_lst, t_o = generate_input(10)
        for index in range(len(input_lst)):
            train_backprop(input_lst[index], t_o[index], weights)
        #train_backprop_batch(input_lst,t_o,weights)
        # count+=1
