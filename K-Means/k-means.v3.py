...
def stream(pix):
    ...
    for row in range(height):
        for col in range(width):
            ...
            flood(pix,row,col,vis,set_color,label)
            # print('what we doin')
            label+=1
    ...
    regions_lst = []
    for component in range(label):
        count = 0
        for row in range(width):
            for col in range(height):
                if vis[row][col] == component: count += 1
        regions_lst += [count]
    print('Region counts: ' + str(regions_lst)[1:-1])

def flood(pix,rw,cl,vis,set_color,label):
    #out of bounds
    if not 0<=rw<height or not 0<=cl<width: return
    #visited
    if vis[rw][cl]!=-1: return
    #wrong color
    if pix[cl,rw]!=set_color: return
    vis[rw][cl]=label

    for i in range(len(di)):
        flood(pix,rw+di[i],cl+dj[i],vis,set_color,label)
...
def parse_input():
    ...
    #size
    image_stats['Size'] = str(height)+' x '+str(width)
    #pixel count
...
def create_color_dict(pix):
    ...
    image_stats['Most common pixel'] = str(max_color)
...
def find_mean(color_lst):
    ...
    for color in color_lst:
        for i in range(len(color)):
            total_color[i]+=color[i]/len(color_lst)
...
def bucket_averages(bucketed_colors):
    k_lst = []
    for bucket in bucketed_colors:
        k_lst.append(find_mean(bucket))
    return k_lst
...
def main():
    ...
    old_k_lst = random.sample(list(color_dict), my_k)
    k_lst = random.sample(list(color_dict), my_k)
    bucketed_colors = []
    while old_k_lst!=k_lst:
        old_k_lst=k_lst
        bucketed_colors = find_closest_centers(color_dict,old_k_lst)
        k_lst=bucket_averages(bucketed_colors)
    ...
    img.save('four_color_dog.png','PNG')
    stream(pix)
