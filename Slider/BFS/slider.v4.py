...
def main():
    if args:
        ...
        for i in range(PUZZLES):
            ...
            if not (i + 1) % (PUZZLES//20): print("*", end='', flush=True)

        endTime = time.process_time() - startTime
        print()
        print('Total Time: ' + '{0:.3g}'.format(endTime) + 's')
        print('Average Impossible time: ' + '{0:.3g}'.format(statsCtr[2] / statsCtr[0]) + 's')
        print('Impossible count: ' + str(statsCtr[0]))
        print('Average path length: ' + '{0:.3g}'.format(statsCtr[4] / (PUZZLES-statsCtr[0])))
        print('Average Possible Time: ' + '{0:.3g}'.format(statsCtr[3] / statsCtr[1]) + 's')
