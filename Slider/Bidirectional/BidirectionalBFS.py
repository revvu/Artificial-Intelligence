...
def solve(root, goal, width):
    ...
    index = 0
    indexTwo = 0
    while index<len(parseMe) and indexTwo<len(parseMeTwo):
        item = parseMe[index]
        index+=1
        if item in dctSeenTwo:
            return len(parsePath(dctSeen,item)+parsePath(dctSeenTwo,item)[1:])
        s = item.index('_')
        ...
        itemTwo = parseMeTwo[indexTwo]
        indexTwo+=1
        if itemTwo in dctSeen:
            return len(parsePath(dctSeen,itemTwo)[:-1]+parsePath(dctSeenTwo,itemTwo)[1:])
        s = itemTwo.index('_')
        ...
        for val in indexTable[s]:
            ...
            if nbr not in dctSeenTwo:
                parseMeTwo.append(nbr)
                dctSeenTwo[nbr] = itemTwo
    return -2
