...
def neighbors(str, width):
    ...
    lst = []
    if col > 0:
        lst.append((swap(str, s-1, s), s-1, s))
    if col < width-1:
        lst.append((swap(str, s, s+1), s+1, s))
    if row > 0:
        lst.append((swap(str, s-width, s), s-width, s))
    if row < len(str)//width-1:
        lst.append((swap(str, s, s+width), s+width, s))
    return lst
...
def solve(root, goal, width):
    ...
    #Create openSet
    openSet = [(manhattan(root, goal, width), root, 0)]
    #Create closedSet
    closedSet = {}
    ...
    while openSet:
        ...
        for nbr in neighbors(pzl, width):
            ...
            md += -single_manhattan(nbr[1],i,width) + single_manhattan(nbr[2],i,width)
            estimate = 1 + md
            priorityInsert(openSet, (estimate, nbr[0], level+1))
    return -1
