...
def solve(root, goal, width):
    ...
    #Create openSet
    #you will always get either f or f+2. once f is finished, shift.
    ...
    openSet, backSetTwo = [(root, 0, root.find('_'), -1)], []
    #Create closedSet
    closedSet = {}
    ...
    while openSet or backSetTwo:
        #find pzl with lowest estimate in openSet
        if not openSet:
            openSet, backSetTwo = backSetTwo, []

        pzl, level, uPos, gpPos = openSet.pop()
        ...
        for nextPos, row in indexTable[uPos]:
            #skip grandparents
            if nextPos == gpPos: continue
            ...
            if row: md = abs(uPos%width - i%width) - abs(nextPos%width - i%width)+1
            else: md = abs(uPos//width - i//width) - abs(nextPos//width - i//width)+1
            ...
            if md: backSetTwo.append((nbr, level+1, nextPos, uPos))
            else: openSet.append((nbr, level+1, nextPos, uPos))
