...
def solve(root, goal, width):
    ...
    while openSet or backSetOne or backSetTwo:
        ...
        for nbr in neighbors(pzl, indexTable):
            if nbr[0] == goal:
                return level+1
            ...
            estimate = 1 + md
            if estimate == est: openSet.append((estimate, nbr[0], level+1))
            elif estimate == est+1: backSetOne.append((estimate, nbr[0], level+1))
            else: backSetTwo.append((estimate, nbr[0], level+1))
    return -1
