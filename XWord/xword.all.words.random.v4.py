...
def parse_word_dict(word_lengths):
    ...
    for base_word in gWORD_LST:
        ...
        for index in range(word_length):
            ...
            if slate in gWORD_DICTIONARY:
                gWORD_DICTIONARY[slate]+=[base_word]
...
def parse_skeleton(pzl, word_dictionary, used_words):
    ...
    for row in range(gHEIGHT):
        ...
        while col<gWIDTH:
            if pzl[row*gWIDTH+col]=='#':
                if inWord:
                    merge_constraints_recur(word, word_dictionary)
...
def final_check(pzl):
    ...
    for row in range(gHEIGHT):
        ...
        while col<gWIDTH:
            if pzl[row*gWIDTH+col]=='#':
                if inWord:
                    if word in used_words: return False
                    used_words.add(word)
                inWord = False
            else:
                if not inWord:
...
def brute_force_words(pzl, word_dictionary, used_words):
    ...
    if not skeleton:
        #solved
        #if not final_check(pzl): print('False')
        return pzl
...
def merge_constraints_recur(target_constraint, copied_dict):
    ...
    if new_constraint1 not in copied_dict:
        copied_dict[target_constraint]=[]
        return []
    ...
    copied_dict[target_constraint]=ret_lst
    return ret_lst
