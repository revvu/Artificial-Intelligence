...
gEDGESET = set()
gWORD_LST = []
gWORD_HORIZONTAL_CONSTRAINTS = []
gWORD_VERTICAL_CONSTRAINTS = []
gWORD_DICTIONARY = {}
gBINARY_DICT = {}
...
def lookup_constraints():
    global gVERTICAL_CONSTRAINTS, gHORIZONTAL_CONSTRAINTS, gWORD_HORIZONTAL_CONSTRAINTS, gWORD_VERTICAL_CONSTRAINTS
    #indexed board
    ...
    for idx in range(0, gHEIGHT):
        gWORD_HORIZONTAL_CONSTRAINTS+=[indexed_board[idx*gWIDTH:idx*gWIDTH+gWIDTH]]

    #word vertical
    for idx in range(0, gWIDTH):
        vertical_constraint_lst = []
        for index in range(idx, len(indexed_board), gWIDTH): vertical_constraint_lst+=[indexed_board[index]]
        gWORD_VERTICAL_CONSTRAINTS+=[vertical_constraint_lst]
...
def parse_word_dict(min_word_length,max_word_length):
    global gWORD_DICTIONARY
    build_binary_dict(max_word_length)

    for base_word in gWORD_LST:
        word_length = len(base_word)
        if word_length>max_word_length or word_length<min_word_length: continue
...
def find_min_word_length(pzl):
    min_word_length = max(gHEIGHT,gWIDTH)
    for row in range(gHEIGHT):
        count = 0
        for col in range(gWIDTH):
            if pzl[row*gWIDTH+col]=='#':
                min_word_length = min(count, min_word_length)
                count = 0
            else: count+=1
        min_word_length = min(count, min_word_length)
    for col in range(gWIDTH):
...
def solve_words(pzl):
    parse_word_dict(find_min_word_length(pzl),find_max_word_length(pzl))
    for row in range(gHEIGHT):
        constraint = ''.join([pzl[i] for i in gWORD_HORIZONTAL_CONSTRAINTS[row]]).replace('#','')
        word = random.choice(gWORD_DICTIONARY[constraint])
        col = 0
        while pzl[row*gWIDTH+col]=='#': col+=1
        place_words(pzl,row,col,word,False)
