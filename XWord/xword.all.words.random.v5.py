...
def print_puzzle(pzl):
    if not pzl: print('no puzzle found')
    else:
...
def parse_word_dict(word_lengths):
    ...
    for base_word in gWORD_LST:
        ...
        my_word_dict['-'*word_length]+=[base_word]

        for index in range(word_length):
            slate = ''.join(['-' if i != index else base_word[i] for i in range(word_length)])
            if slate in my_word_dict:
                my_word_dict[slate] += [base_word]
            else:
                my_word_dict[slate] = [base_word]
...
def parse_skeleton(pzl, word_dictionary, used_words):
    ...
    for row in range(gHEIGHT):
        ...
        while col<gWIDTH:
            if pzl[row*gWIDTH+col]=='#':
                if inWord:
                    merge_constraints(word, word_dictionary)
                    if '-' not in word and len(word_dictionary[word]) == 0: horizontal_skeleton += [
                        (len(word_dictionary[word]), word_start_index, length, word, False)]

                    if '-' in word:
                        horizontal_skeleton+=[(len(word_dictionary[word]),word_start_index,length,word, False)]
                ...
                if not inWord:
                    ...
                    length+=1
            col+=1
        if (word_start_index, length, word) not in horizontal_skeleton:
            merge_constraints(word, word_dictionary)
            if '-' not in word and len(word_dictionary[word])==0: horizontal_skeleton += [(len(word_dictionary[word]),word_start_index, length, word, False)]
            if '-' in word:
                horizontal_skeleton += [(len(word_dictionary[word]),word_start_index, length, word, False)]
    ...
    for col in range(gWIDTH):
        ...
        while row<gHEIGHT:
            if pzl[row*gWIDTH+col]=='#':
                if inWord:
                    merge_constraints(word, word_dictionary)
                    if '-' not in word and len(word_dictionary[word]) == 0: vertical_skeleton += [
                        (len(word_dictionary[word]), word_start_index, length, word, True)]
                    #if '-' not in word and word in used_words: vertical_skeleton += [(0, word_start_index, length, word, True)]
                    if '-' in word:
                        vertical_skeleton+=[(len(word_dictionary[word]),word_start_index,length,word, True)]
        ...
        if (word_start_index, length, word) not in vertical_skeleton:
            merge_constraints(word, word_dictionary)
            if '-' not in word and len(word_dictionary[word])==0: vertical_skeleton += [(len(word_dictionary[word]),word_start_index, length, word, False)]
            #if '-' not in word and word in used_words: vertical_skeleton += [(0, word_start_index, length, word, True)]
            if '-' in word:
                vertical_skeleton += [(len(word_dictionary[word]), word_start_index, length, word, True)]
...
def brute_force_words(pzl, word_dictionary, used_words):
    horizontal_skeleton, vertical_skeleton = parse_skeleton(pzl, word_dictionary, used_words)
    ...
    poss_word_count,start_index,word_length,constraint,verticalness = min(skeleton)
    if poss_word_count==0: return []

    random.shuffle(word_dictionary[constraint])

    for word in word_dictionary[constraint]:
        if word in used_words: continue
        new_pzl = [i for i in pzl]
        place_words(new_pzl,start_index,word,verticalness)
        used_words.add(word)
        bF = brute_force_words(new_pzl, word_dictionary,used_words)
        if bF: return bF
        used_words.remove(word)

def solve_words(pzl):
    word_dictionary = parse_word_dict(find_word_lengths(pzl))
    used_words = set()
    return brute_force_words(pzl, word_dictionary,used_words)

def merge_constraints(target_constraint, my_word_dict):
    if target_constraint in my_word_dict:
        return my_word_dict[target_constraint]

    constraint_lst = []

    for index in range(len(target_constraint)):
        if target_constraint[index]=='-': continue
        base_constraint = ['-']*len(target_constraint)
        base_constraint[index]=target_constraint[index]
        constraint = ''.join(base_constraint)
        if constraint not in my_word_dict:
            my_word_dict[target_constraint]=[]
            return
        constraint_lst += [(len(my_word_dict[constraint]), constraint)]

    min_length, min_constraint = min(constraint_lst)
    final_lst = []
    for word in my_word_dict[min_constraint]:
        good_word = True
        for length, constraint in constraint_lst:
            if word not in my_word_dict[constraint]:
                good_word = False
                break
        if not good_word: continue
        final_lst+=[word]

    my_word_dict[target_constraint]=final_lst
