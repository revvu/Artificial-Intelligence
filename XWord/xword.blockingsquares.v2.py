...
gSEEDS, g180LST, gCONSTRAINT_LST_LST, gCONSTRAINT_ID = [],[],[],[]
gHORIZONTAL_CONSTRAINTS = []
gVERTICAL_CONSTRAINTS = []

def parse_input():
    global gBLOCKCOUNT, gSEEDS, g180LST, gHALF_LENGTH
    for arg in args:
...
def lookup_constraints():
    global gCONSTRAINT_LST_LST, gCONSTRAINT_ID, gVERTICAL_CONSTRAINTS, gHORIZONTAL_CONSTRAINTS
    #indexed board
    indexed_board = [i for i in range(gHEIGHT*gWIDTH)]
    #horizontal constraints
    for idx in range(0, gHEIGHT//2):
        gCONSTRAINT_LST_LST+=[indexed_board[idx*gWIDTH:idx*gWIDTH+gWIDTH]]
    ...
    if gHEIGHT%2:
        ...
        gCONSTRAINT_LST_LST+=[horizontal_constraint_lst]
    ...
    for idx in range(0, (gWIDTH+1)//2):
        ...
        gCONSTRAINT_LST_LST+=[vertical_constraint_lst]
        gVERTICAL_CONSTRAINTS+=[vertical_constraint_lst]

    gCONSTRAINT_ID = [[]]*gHALF_LENGTH
    #go through and add IDs (lookup so it's okay to be slow)
    for idx in range(gHALF_LENGTH):
        for lst_idx in range(len(gCONSTRAINT_LST_LST)):
            if idx in gCONSTRAINT_LST_LST[lst_idx]: gCONSTRAINT_ID[idx]+=[lst_idx]
...
def floodfill(pzl):
    ...
    vis = [blocked_pzl[idx]=='#' for idx in range(len(blocked_pzl))]
    single_component = True
    for idx in range(len(blocked_pzl)):
        if vis[idx]: continue
        if not single_component: return False
        floodfill_recur(idx,vis,dpos)
        single_component=False
    return True

def floodfill_recur(index,vis,dpos):
    if vis[index]: return
    vis[index]=True
    for pos in dpos:
        floodfill_recur(index+pos,vis,dpos)

def brute_force_blocks(pzl, index_lst):
    #verify validity in above step
    if pzl[:-1].count('#')==gBLOCKCOUNT//2: return pzl
    if pzl[:-1].count('#')>gBLOCKCOUNT//2 or not index_lst: return []
    ...
    while index_lst:
        ...
        if new_pzl and floodfill(new_pzl) and mid_connect_check(new_pzl):
            new_index_lst = [i for i in index_lst]
            bF = brute_force_blocks(new_pzl, new_index_lst)
            if bF: return bF

        #if its not connected, choose one of them to keep
        pzl[temp]='*'
        index_lst= [temp]+index_lst
...
def solve(pzl):
    ...
    index_lst = lookup_index_lst(pzl)[::-1]
    #solve blocks
    pzl = brute_force_blocks(pzl,index_lst)
    #assemble puzzle
    complete_pzl = pzl[:-1]+[pzl[-1]]*(not gWIDTH%2 and not gHEIGHT%2)+pzl[::-1]
    complete_pzl = place_words(complete_pzl)
    print_puzzle(complete_pzl)
