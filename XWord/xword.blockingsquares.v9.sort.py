...
def lookup_index_lst(pzl):
    #change to sorted by distance to the nearest block
    lst_to_sort = []
    dpos = [1,-1,gWIDTH,-gWIDTH]
    for i in range(len(pzl)):
        if pzl[i]!='*':continue
        #find the nearest block/edge
        min_dist = gWIDTH # the actual distance will always be lower
        for direction in dpos:
            multiple = 0
            index = i+direction*multiple
            while index not in gEDGESET and pzl[index]!='#':
                multiple+=1
                index = i+direction*multiple
                if index >= len(pzl): break
            if index >= len(pzl): continue
            if index in gEDGESET: min_dist = min(min_dist,multiple+1)
            if pzl[index]=='#': min_dist = min(min_dist,multiple)
        lst_to_sort+=[(min_dist,i)]
    lst_to_sort = sorted(lst_to_sort)
    return [z[1] for z in lst_to_sort]
...
def main():
    if not args:
        print('Please input xword dimensions, blocking square count, and seed strings')
    else:
