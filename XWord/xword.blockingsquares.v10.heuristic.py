...
def find_neighbors(pzl):
    possible_block = [i for i in range(len(pzl)) if pzl[i]=='*']
    neighbors = []
    while possible_block:
        temp = possible_block.pop()
        new_pzl = [i for i in pzl]
        new_pzl[temp]='#'
        new_pzl = constrain_word_length(new_pzl)
        for neighbor in floodfill(new_pzl):
...
def dfs_blocks(pzl):
    ...
    if pzl.count('#')>(1+gBLOCKCOUNT)//2 or not pzl.count('*'): return []

    #index_lst = lookup_index_lst(pzl)
    index_lst = [i for i in range(len(pzl)) if pzl[i]=='*']
