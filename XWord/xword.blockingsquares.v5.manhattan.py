...
def lookup_index_lst(pzl):
    # sort by manhattan distance to the center
    ret = []
    for i in range(len(pzl)):
        if pzl[i]=='*':
           ret+=[(abs(gWIDTH//2-i%gHEIGHT)+abs(gHEIGHT//2-i//gWIDTH),i)]
    ret.sort()
    return [z[1] for z in ret]
...
def find_neighbors(pzl):
    queue = [pzl]
    finished = []

    while queue:
        item = queue.pop()
        new_pzl = constrain_word_length(item)
        if new_pzl and mid_connect_check(new_pzl):
            lst = floodfill(new_pzl)
            if len(lst)==1:
                finished+=lst
            else:
                queue+=lst
...
def brute_force_blocks(pzl, index_lst):
    ...
    while index_lst:
        ...
        temp = index_lst.pop()
        pzl[temp]='#'
        for neighbor in find_neighbors(pzl):
            new_index_lst = [i for i in index_lst]
            bF = brute_force_blocks(neighbor, new_index_lst)
            if bF: return bF
        pzl[temp]='-'
    print('we in the bad')
