...
def count_stable(brd, tkn):
    return sum(pos_stable(brd, tkn, i) for i in range(64))
...
def edge_heuristic(brd,tkn):
    tkn_cnt = count_edges(brd, tkn)
    eTkn_cnt = count_edges(brd, oppose(tkn))
    return 100.0/24.0*(tkn_cnt-eTkn_cnt)
...
def evaluate_pos(brd,tkn):
    ...
    value += 0*xc_squares_heuristic(brd, tkn)
    value += 0*edge_heuristic(brd,tkn)
    value += 0*wedged_edge_heuristic(brd, tkn)
