...
def find_best_move(othello_board, token, lst_of_moves):
    ...
    for edge in edge_dict:
        ...
        direction = 1
        #left right?
        if 8 in directions_dct[edge] and -8 in directions_dct[edge]:
            direction = 8
        #if safe play it, else put it in a list to remove it

        #what is the last that it comes across?
        hitdot = False
        point = edge
        while direction in directions_dct[point] and point==token:
            point = point + direction
        #dead end? safe to play
        if point == token:
            return edge
        if point == '.': hitdot = True
        ...
        if point == token:
            return edge
        #not safe
        if hitdot and point==oppose(token):
            bad_edges.append(edge)
        elif not hitdot and point=='.':
            bad_edges.append(edge)

    if len(bad_edges) < len(lst_of_moves):
        for possible_move in bad_edges:
            del lst_of_moves[possible_move]

    return list(lst_of_moves.keys())[0]
