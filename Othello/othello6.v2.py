    ...
    while True:
        ...
        if player==token:
            if board.count('.') < LIMIT_AB:
                move = alphabeta(board, player, -65, 65)[-1]
                #print(alphabeta(board, player, -65, 65))
            else:
...
def individualMoveProcessing():
    ...
    if not moves:
        ...
        if board.count('.') < LIMIT_AB:
            nm = alphabeta(board, token, -65, 65)
...
def negamax(brd, tkn):
    ...
    if not possible_moves:
        possible_moves = find_possible_moves(brd, eTkn)
        #game over
        if not possible_moves:
    ...
    for mv in list(possible_moves.keys()):
        ...
        if (newBrd, eTkn) not in negamax_dict:
            negamax_dict[(newBrd, eTkn)] = nm
        if -nm[0] > bestSoFar[0]:
...
def alphabeta(brd, tkn, lowerBnd, upperBnd):
    ...
    if not possible_moves:
        ...
        #if ab is passing back a move list, consider it
        if len(ab)>1:
            return [-ab[0]] + ab[1:] + [-1]
        #otherwise it's just a vile score
        return [-ab[0]]

    if len(possible_moves)==1:
        move = list(possible_moves.keys())[0]
        newBrd = make_move(brd, tkn, move, possible_moves)
        return [newBrd.count(tkn) - newBrd.count(eTkn)] + [move]

    #try the best move first
    best = [lowerBnd - 1]

    # mv = find_best_move(brd, tkn, possible_moves)
    # newBrd = make_move(brd, tkn, mv, possible_moves)
    ...
    # if mv:=play_wedged_edge(brd, tkn, possible_moves):
    #     newBrd = make_move(brd, tkn, mv, possible_moves)
    #     ab = alphabeta(newBrd, eTkn, -upperBnd, -lowerBnd)
    #     score = -ab[0]
    #     if score >= lowerBnd:
