...
dir_dct, negamax_dct, mv_lst_dct, stable_dict = {}, {}, {}, {}
...
def count_corner(brd,tkn):
    return sum(brd[i]==tkn for i in corner_set)
...
def pos_stable(brd,tkn,idx):
    return brd[idx]==tkn and is_move_stable(brd,tkn,idx)

def count_stable(brd, tkn):
    if (brd,tkn) in stable_dict: return stable_dict[(brd,tkn)]
    stable_dict[(brd,tkn)]=sum(pos_stable(brd,tkn,i) for i in range(64))
    return stable_dict[(brd,tkn)]
...
def preferred_move(brd, tkn, mv_lst):
    if brd.count('.')<LIMIT_AB: return alphabeta(brd, tkn, -65, 65,LIMIT_AB, 0)[-1]
    return alphabeta(brd, tkn, -65, 65,LIMIT_MIDGAME_AB, 0)[-1]
...
def playGame(tkn):
    ...
    while brd.count('.'):
        ...
        if cur==tkn: mv = preferred_move(brd,cur,mv_lst)
...
def print_AB(brd, tkn):
    ...
    nm = alphabeta(brd, tkn, -65, 65, LIMIT_AB,0)
...
def individualMoveProcessing():
    ...
    if not mvs and brd.count('.'):
        print("My move is: " + str(find_best_move(brd, tkn, mv_lst)))
        if brd.count('.')<LIMIT_AB: print_AB(brd, tkn)
...
def mv_heuristic(brd,tkn):
    tkn_cnt = brd.count(tkn)
    eTkn_cnt = brd.count(oppose(tkn))
    if tkn_cnt>eTkn_cnt:
...
def stable_mv_heuristic(brd,tkn):
    tkn_cnt = count_stable(brd, tkn)
    eTkn_cnt = count_stable(brd,oppose(tkn))
    if tkn_cnt>eTkn_cnt:
        return (100.0*tkn_cnt)/(tkn_cnt+eTkn_cnt)
    if tkn_cnt<eTkn_cnt:
        return -(100.0*eTkn_cnt)/(tkn_cnt+eTkn_cnt)
...
def x_squares_heuristic(brd, tkn):
    prblm = {0:9, 7:14, 56:49, 63:54}
    eTkn = oppose(tkn)
    tkn_cnt = 0
    eTkn_cnt = 0
...
def c_squares_heuristic(brd, tkn):
    prblm = {0:[1,8], 7:[6,15], 56:[48,57], 63:[55,62]}
    eTkn = oppose(tkn)
    tkn_cnt = 0
    eTkn_cnt = 0
...
def wedged_edge_heuristic(brd, tkn):
    dct_lst = [1, 7, 8, 9]
    eTkn = oppose(tkn)
    tkn_cnt = 0
    eTkn_cnt = 0
    ...
    for idx in range(64):
        if idx not in edge_set: continue
        if brd[idx]=='.': continue
        for dct in dct_lst:
            if brd[idx]==tkn and dct in dir_dct[idx] and -dct in dir_dct[idx] and brd[idx + dct] == brd[idx - dct] == eTkn:
                tkn_cnt+=1
            elif brd[idx]==eTkn and dct in dir_dct[idx] and -dct in dir_dct[idx] and brd[idx + dct] == brd[idx - dct] == tkn:
                eTkn_cnt+=1
...
def safe_edge_heuristic(brd, tkn):
    dct_lst = [1, 7, 8, 9]
    eTkn = oppose(tkn)
    tkn_cnt = 0
    eTkn_cnt = 0
    ...
    for idx in range(64):
        ...
        if brd[idx]=='.': continue
        for dct in dct_lst:
            if brd[idx]==tkn and dct in dir_dct[idx] and -dct in dir_dct[idx] and brd[idx + dct] == brd[idx - dct] == '.':
                tkn_cnt+=1
            elif brd[idx]==eTkn and dct in dir_dct[idx] and -dct in dir_dct[idx] and brd[idx + dct] == brd[idx - dct] == '.':
                eTkn_cnt+=1
...
def positional_heuristic(brd,tkn):
    eTkn = oppose(tkn)
    tkn_cnt = 0
    eTkn_cnt = 0

    for corner in corner_set:
        for dct in dir_dct[corner]:
            if brd[corner+dct]==tkn: tkn_cnt+=1
            elif brd[corner+dct]==eTkn: eTkn_cnt+=1
    return -12.5*(tkn_cnt-eTkn_cnt)
...
def evaluate_pos(brd,tkn):
    ...
    value = 10000*corner_heuristic(brd,tkn)
    value += 1000*x_squares_heuristic(brd, tkn)
    value += 100*c_squares_heuristic(brd, tkn)
    value += 10*wedged_edge_heuristic(brd, tkn)
    value += 1*safe_edge_heuristic(brd,tkn)
    value += 0.1*stable_mv_heuristic(brd,tkn)
    value += 0*mobility_heuristic(brd,tkn)
    value += 0*frontier_heuristic(brd,tkn)
    value += 0*mv_heuristic(brd,tkn)
    value += 0*positional_heuristic(brd,tkn)
    value += 0*(brd.count(tkn)-brd.count(eTkn))

    return int(value*64/1111111.1)

# returns best score + reversed move sequence
def alphabeta(brd, tkn, lowerBnd, upperBnd, count, inc):
    eTkn = oppose(tkn)

    if count == inc and inc!=0:
        # if count%2: return [evaluate_pos(brd, tkn)]
    ...
    if not possible_moves:
        ...
        ab = alphabeta(brd, eTkn, -upperBnd, -lowerBnd, count,inc-1)
        if len(ab)>1:
            return [-ab[0]] + ab[1:] + [-1]
    ...
    #try the best move first
    best = [lowerBnd - 1]

    for mv in possible_moves:
        if mv in corner_set:
            newBrd = make_move(brd, tkn, mv, possible_moves)
            ab = alphabeta(newBrd, eTkn, -upperBnd, -lowerBnd, count, inc)
            score = -ab[0]
