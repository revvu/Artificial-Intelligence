...
def preferred_move(brd, tkn):
    ...
    return alphabeta(brd, tkn, -65, 65,LIMIT_MIDGAME_AB, True)[-1]
...
#include duplicates because that means more is known
...
def isWinning(brd, tkn, mv):
    mv_lst = poss_mvs(brd,tkn)
    brd = make_move(brd, tkn, mv, mv_lst)
    depth = brd.count('.')
    ab = alphabeta(brd,oppose(tkn),-65,65,depth,True)
    return ab[0]<0 # because this is enemy score
    ...
    def best_strategy(self, board, player, best_move, running):
        ...
        if running.value:
            ...
            if board in opening_book: best_move.value = random.choice(opening_book[board])
                ...
                move = alphabeta(board, player, -65, 65, 3, False)[-1]
                best_move.value = move
                mv_lst = [*poss_mvs(board, player)]
                if len(mv_lst)==1: return
                if LIMIT_AB<=board.count('.')<WINNING_DEPTH:
                    if isWinning(board,player,move): return
                    mv_lst.remove(move)
                    best_move.value = random.choice(mv_lst)
                    for mv in mv_lst:
                        if isWinning(board, player, mv):
                            best_move.value = mv
                            break
                elif board.count('.')<LIMIT_AB:
                    best_move.value = alphabeta(board, player, -65, 65, LIMIT_AB, False)[-1]
                else:
                    for depth in range(6,35):
