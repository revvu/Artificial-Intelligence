...
def individualMoveProcessing():
    ...
    if not mvs and brd.count('.'):
        if brd in opening_book:
            move = opening_book[brd][0]
            print("My move is " + str(move))
        else:
            if brd.count('.') < LIMIT_AB:
                print_AB(brd, tkn)
            else:
                move = alphabeta(brd, tkn, -65, 65, LIMIT_MIDGAME_AB, False)[-1]
                print("My move is " + str(move))
...
class Strategy:
    ...
    def best_strategy(self, board, player, best_move, running):
        ...
        if running.value:
            ...
            if board in opening_book: best_move.value = opening_book[board][0]
            else:
                if board.count('.')<LIMIT_AB:
                    best_move.value = alphabeta(board, player, -65, 65, LIMIT_AB, False)[-1]
                else:
                    best_move.value = alphabeta(board, player, -65, 65, LIMIT_MIDGAME_AB, False)[-1]
