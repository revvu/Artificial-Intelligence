...
def lookup_board_directions():
    #corners
    global directions_dct, edge_set, corner_set, almost_corner_set, danger_zone
    directions_dct = {
        0: {1, 8, 9},
        7: {-1, 7, 8},
        56: {-8, -7, 1},
    ...
    danger_zone = {9, 10, 11, 12, 13, 14, 17, 22, 25, 30, 33, 38, 41, 46, 49, 50, 51, 52, 53, 54}
    #edges
    for i in range(1, 7):
...
def negamax(brd, tkn):
    ...
    for mv in list(possible_moves.keys()):
        newBrd, useless_token = make_move(brd,tkn,mv, possible_moves)
...
def main():
    ...
    if args:
        brd, tkn = parse_input()
        print(find_best_move(brd, tkn, find_possible_moves(brd, tkn)))
        return
